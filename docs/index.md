...
- **Cencori-native** — sessions, turns, and streaming run through the Cencori Sessions API. No build step required.
...
- **Local-first dev** — `arcie dev` starts a Next.js web UI with hot-reload. The local LLM server supports OpenAI, Anthropic, Groq, DeepSeek, Mistral, Google, and Together — no Cencori dependency required for development.
