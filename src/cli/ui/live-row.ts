...
import { sliceVisible, visibleLength } from "./terminal-text";
...
function renderProgressRow(
  ...
  if (prefixWidth >= maxWidth) {
    ...
    return fitted.startsWith(glyph) ? `${chalk.green(glyph)}${fitted.slice(glyph.length)}` : fitted;
  }

  const fittedDetail = hasDetail ? fitProgressText(` ${detail}`, maxWidth - prefixWidth) : "";
  const styledDetail = fittedDetail === "" ? "" : chalk.dim(fittedDetail);
  return `${chalk.green(glyph)}${prefix.slice(glyph.length)}${styledDetail}`;
}
...
export function startCliLiveRow(
  ...
  const animate = output.isTTY === true;
